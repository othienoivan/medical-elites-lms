              ...
              <div className="flex flex-col gap-5 lg:flex-row lg:justify-between">
                <div className="flex-1">
                  <div className="flex flex-wrap gap-2">
                    ...
                    {renderPerformanceBadge({ awarded: manualMark?.marksAwarded ?? answer.marksAwarded, maximum: question?.marks ?? 0, objectivelyCorrect: answer.isCorrect === true })}
                  </div>
...
function renderPerformanceBadge({ awarded, maximum, objectivelyCorrect }: { awarded: number; maximum: number; objectivelyCorrect: boolean }) {
  if (objectivelyCorrect || (maximum > 0 && awarded >= maximum)) {
    return <span className="rounded-full bg-green-100 px-3 py-1 text-sm font-semibold text-green-700">Fully correct</span>;
  }
  if (awarded > 0) {
    return <span className="rounded-full bg-amber-100 px-3 py-1 text-sm font-semibold text-amber-800">Partially correct</span>;
  }
  return <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-700">Needs improvement</span>;
}