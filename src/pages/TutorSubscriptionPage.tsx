...
export default function TutorSubscriptionPage() {
  ...
  useEffect(() => {
    ...
    setReconciling(true);
    setMessage("Verifying your subscription payment…");
  ...
  const limits = [
    ...
    { label: "Storage", value: formatBytes(plan.limits.storageBytes), icon: HardDrive },
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-3xl border bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-black text-slate-950">{loading ? "Loading…" : plan.name}</h2>
        ...
        <section className="grid gap-5 lg:grid-cols-2">
          ...
          <div className="rounded-3xl border border-cyan-200 bg-cyan-50 p-6">
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <button onClick={() => void refreshSubscription()} disabled={reconciling} className="inline-flex items-center gap-2 rounded-xl border border-cyan-300 bg-white px-4 py-3 font-bold text-cyan-800 disabled:opacity-60"><RefreshCw size={17}/>{reconciling ? "Verifying…" : "Refresh subscription"}</button>
              ...
              {!isFree && !activeSubscription?.cancelAtPeriodEnd && <button onClick={() => void cancelAtPeriodEnd()} disabled={cancelling} className="rounded-xl border border-rose-200 bg-white px-4 py-3 font-bold text-rose-700 disabled:opacity-60">{cancelling ? "Scheduling…" : "Cancel at period end"}</button>}
            </div>
          </div>
        </section>
        ...
        {!isFree && renewWindowOpen && (
          <section className="rounded-3xl border border-amber-200 bg-amber-50 p-6">
            <div className="flex items-start gap-3"><CalendarClock className="mt-1 text-amber-700"/><div><h3 className="text-lg font-black text-slate-950">Renewal window is open</h3><p className="mt-1 text-sm text-slate-700">Your current period ends on {formatDate(activeSubscription?.currentPeriodEnd)}. Renewing now extends the same plan from the current period end.</p><button onClick={() => void startPlanCheckout(plan)} disabled={checkoutPlanId !== null} className="mt-4 rounded-xl bg-amber-700 px-4 py-3 font-bold text-white disabled:opacity-60">{checkoutPlanId === plan.id ? "Opening checkout…" : `Renew ${plan.name}`}</button></div></div>
          </section>
        )}
        ...
        <section className="rounded-3xl border bg-white p-6 shadow-sm">
          ...
          {plansLoading
            ? <p className="mt-5 text-slate-500">Loading upgrade plans…</p>
            : upgradePlans.length === 0
              ...
              : <div className="mt-5 grid gap-4 lg:grid-cols-3">{upgradePlans.map((item) => <div key={item.id} className="rounded-2xl border p-5"><p className="text-sm font-bold uppercase tracking-wide text-cyan-700">{item.billingInterval}</p><h4 className="mt-2 text-xl font-black">{item.name}</h4><p className="mt-2 text-2xl font-black">{formatMoney(item.priceMinor, item.currency)}</p><p className="mt-2 text-sm text-slate-600">{item.description}</p><button disabled={checkoutPlanId !== null} onClick={() => void startPlanCheckout(item)} className="mt-5 w-full rounded-xl bg-cyan-700 px-4 py-3 font-bold text-white disabled:opacity-60">{checkoutPlanId === item.id ? "Opening checkout…" : `Upgrade to ${item.name}`}</button></div>)}</div>}
        </section>
      </div>
    </TutorLayout>
  );
