...
export default function AdminModulesPage() {
  ...
  const reload = useCallback(async () => { setLoading(true); try { const [moduleRows, programmeRows, courseRows] = await Promise.all([getAllModules(), getAllProgrammes(), getAllCourseUnits()]); setRecords(moduleRows); setProgrammes(programmeRows); setCourseUnits(courseRows); } finally { setLoading(false); } }, []);
  useEffect(() => {
