  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await saveSubscriptionPlan({
        ...
        limits: { maxStudents:Number(fd.get("maxStudents")), maxTutors:Number(fd.get("maxTutors")), maxCourseUnits:Number(fd.get("maxCourseUnits")), storageBytes:Number(fd.get("storageGb")) < 0 ? -1 : Number(fd.get("storageGb"))*1024**3, monthlyAiCredits:Number(fd.get("monthlyAiCredits")) },
  ...
  return <PlatformLayout title="Subscription Plans" subtitle="Configure pricing, trials, entitlements and usage limits through trusted backend controls.">
    <div className="grid gap-6 xl:grid-cols-[1.2fr_1fr]">
      ...
      <PlatformCard title={selected?"Edit plan":"Create plan"}><form key={selected?.id ?? "new-plan"} onSubmit={submit} className="space-y-3">
        ...
        <div className="rounded-xl bg-cyan-50 p-3 text-xs font-semibold text-cyan-900">Capacity limits are plan data, not hard-coded access rules. Use -1 for Unlimited. Avoid 0 on paid plans unless the feature must be disabled.</div>
        <div className="grid grid-cols-2 gap-3">{([['maxStudents','Students'],['maxTutors','Tutors'],['maxCourseUnits','Course units'],['monthlyAiCredits','AI credits']] as const).map(([key,label])=><label key={key} className="text-xs font-semibold text-slate-600">{label} limit<input name={key} type="number" min="-1" defaultValue={current.limits?.[key] ?? emptyLimits[key]} placeholder={`Max ${label}`} className="mt-1 w-full rounded-xl border p-3 text-sm font-normal"/><span className="mt-1 block font-normal text-slate-400">-1 = Unlimited</span></label>)}<label className="text-xs font-semibold text-slate-600">Storage GB<input name="storageGb" type="number" min="-1" defaultValue={(current.limits?.storageBytes ?? emptyLimits.storageBytes)/1024**3} placeholder="Storage GB" className="mt-1 w-full rounded-xl border p-3 text-sm font-normal"/><span className="mt-1 block font-normal text-slate-400">-1 = Unlimited</span></label><label className="text-xs font-semibold text-slate-600">Trial days<input name="trialDays" type="number" min="0" defaultValue={current.trialDays ?? 14} placeholder="Trial days" className="mt-1 w-full rounded-xl border p-3 text-sm font-normal"/></label></div>
