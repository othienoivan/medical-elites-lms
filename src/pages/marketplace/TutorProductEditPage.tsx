...
export default function TutorProductEditPage() {
  ...
  async function save() {
    ...
    try {
      await MarketplaceService.updateProduct(productId, {
        ...
        linkedResourceIds: selectedCourseUnit ? [selectedCourseUnit.id] : [], thumbnailUrl: product.thumbnailUrl?.trim(),
  ...
  if (loading) return <TutorLayout title="Edit Product"><p className="p-6">Loading product…</p></TutorLayout>;
  ...
  return <TutorLayout title="Edit Product" subtitle="Update pricing, content links, visibility and publication status.">
    <div className="mx-auto max-w-5xl space-y-5">
      ...
      <section className="grid gap-5 rounded-2xl border bg-white p-6 md:grid-cols-2">
        ...
        <label className="md:col-span-2"><span className="mb-2 block font-bold">Linked course unit</span><select className="w-full rounded-xl border px-4 py-3" value={product.courseUnitId ?? ""} onChange={(e)=>set("courseUnitId",e.target.value || undefined)}><option value="">No linked course unit</option>{courseUnits.map((c)=><option key={c.id} value={c.id}>{c.code ? `${c.code} — ` : ""}{c.title}</option>)}</select></label>
        ...
        <label className="md:col-span-2"><span className="mb-2 block font-bold">Thumbnail URL</span><input className="w-full rounded-xl border px-4 py-3" value={product.thumbnailUrl ?? ""} onChange={(e)=>set("thumbnailUrl",e.target.value)}/></label>
      </section>
      <div className="flex justify-end"><button type="button" disabled={saving} onClick={()=>void save()} className="inline-flex items-center gap-2 rounded-xl bg-cyan-700 px-5 py-3 font-black text-white disabled:opacity-60"><Save size={18}/>{saving ? "Saving…" : "Save changes"}</button></div>
    </div>
  </TutorLayout>;
}
