...
async function syncQuizPlacement(quizId: string, next: Partial<Quiz>, previous?: Quiz | null) {
  ...
  const passRequired = next.requiresPassForProgression !== false;
  if (next.lessonId) {
    await updateDoc(doc(db, "lessons", next.lessonId), { quizId, quizRequired: passRequired, quizPassMark: next.passMark ?? null, updatedAt: serverTimestamp() });
  } else if (next.moduleId) {
    await updateDoc(doc(db, "modules", next.moduleId), { quizId, quizRequired: passRequired, updatedAt: serverTimestamp() });
  }
}
